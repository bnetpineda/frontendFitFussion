...
export const listGenres = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("https://backendfitfussion-production.up.railway.app/api/genres"); //fetch the products from rest api
...
export const addGenre = (genre) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("https://backendfitfussion-production.up.railway.app/api/genres/create", genre); //create a new product
